...
namespace FronkonGames.Glitches.Hacked
{
  ...
  public sealed partial class Hacked
  {
    ...
    private static readonly Hacked[] NoEffects = new Hacked[0];
    ...
    public static Hacked Instance
    {
      get
      {
        ...
        if (pipelineAsset != null)
        {
          ...
          {
            if (scriptableRendererData.rendererFeatures[i] is Hacked)
              return scriptableRendererData.rendererFeatures[i] as Hacked;
          }
        }
    ...
    /// <remarks> This function use Reflection, so it can be slow. </remarks>
    public static Hacked[] Instances
    {
      get
      {
          ...
          for (int i = 0; i < rendererDataList.Length; ++i)
          {
            if (rendererDataList[i] != null && rendererDataList[i].rendererFeatures.Count > 0)
              foreach (var feature in rendererDataList[i].rendererFeatures)
                if (feature is Hacked)
                  effects.Add(feature as Hacked);
          }
